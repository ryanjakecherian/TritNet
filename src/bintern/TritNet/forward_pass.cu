    ...
    if ((multistream == true) && depth > 0) {                      //minimum depth for multistream is 1: LOADING inputs 2 weights & calcs 1 activation, and DELOADING calcs 2 activations & outputs 1 result. These 2 phases alone sum to 3 layers, i.e. a depth of 1.
        ...
        // INITIALISE DUAL-BUFFER MULTI-STREAM SYSTEM:
            ...
            // allocate buffers (of max required size) 
                if (cudaSuccess!= cudaMalloc(&d_b1_A, n*A_max + T_size) ) {throw std::runtime_error("CUDA memory allocation failed");}; 
                if (cudaSuccess!= cudaMalloc(&d_b2_A, n*A_max + T_size) ) {throw std::runtime_error("CUDA memory allocation failed");};
            ...
            // H2D: A[0]
                if (cudaSuccess!= cudaMemcpy(d_b2_A, A_list[0], n*A_bytesizes[0] + T_size, cudaMemcpyHostToDevice) ) {throw std::runtime_error("CUDA memcpy failed");};
        ...
        // LOADING: phase 2
            ...
            // <<<prop>>>(0)        //S2
                if (cudaSuccess!= cudaMemsetAsync(d_b1_A, 0, n*A_bytesizes[1] + T_size, ProcessStream) ) {throw std::runtime_error("CUDA memset failed");};
                dim3 gridDim(layers[0+1]+1, n);       //i.e. blockIdx.y goes from 0 -> n-1. blockIdx.x goes from 0 -> p/word_size -1    where p is the #cols of the next layer 
            ...
            for(int i = 1; i<depth; i++){
                ...
                if (i%2 == 1){
                    ...
                    // D2H: A[i]            //S1
                        ...
                        cudaMemcpyAsync(A_list[i], d_b1_A, n*A_bytesizes[i] + T_size, cudaMemcpyDeviceToHost, TransferStream);

                    // <<<prop>>>(i)        //S2
                        if (cudaSuccess!= cudaMemset(d_b2_A, 0, n*A_bytesizes[i+1] + T_size) ) {throw std::runtime_error("CUDA memset failed");};
                        gridDim.x = layers[i+1]+1;       //p is the #cols of W[i] (which is the #cols of A[i+1])
                    ...
                    // D2H: A[i]            //S1
                        ...
                        cudaMemcpyAsync(A_list[i], d_b2_A, n*A_bytesizes[i] + T_size, cudaMemcpyDeviceToHost, TransferStream);
    
                    // <<<prop>>>(i)        //S2
                        if (cudaSuccess!= cudaMemset(d_b1_A, 0, n*A_bytesizes[i+1] + T_size) ) {throw std::runtime_error("CUDA memset failed");};
                        gridDim.x = layers[i+1]+1;       //p is the #cols of W[i] (which is the #cols of A[i+1])
                        propagate<T><<<gridDim,blockDim,0,ProcessStream>>>(d_b2_A, d_b1_W, d_b1_A);
        ...
        if ( (depth)%2 == 1) {
            ...
            // DELOADING: phase 1
                // D2H: A[depth]        //S1
                    ...
                    if (cudaSuccess!=cudaMemcpyAsync(A_list[depth], d_b1_A, n*A_bytesizes[depth] + T_size, cudaMemcpyDeviceToHost, TransferStream) ) {throw std::runtime_error("CUDA memcpy failed");};

                // <<<prop>>>(depth)    //S2
                    if (cudaSuccess!= cudaMemsetAsync(d_b2_A, 0, n*A_bytesizes[depth+1] + T_size, ProcessStream) ) {throw std::runtime_error("CUDA memset failed");};
                    gridDim.x = layers[depth+1]+1;       //i.e. blockIdx.y goes from 0 -> n-1. blockIdx.x goes from 0 -> p/word_size -1    where p is the #cols of the next layer 
                    propagate<T><<<gridDim,blockDim,0,ProcessStream>>>(d_b1_A, d_b2_W, d_b2_A);
            ...
            // DELOADING: phase 2
                // D2H: A[depth+1]      //S1
                    ...
                    cudaMemcpyAsync(A_list[depth+1], d_b2_A, n*A_bytesizes[depth+1] + T_size, cudaMemcpyDeviceToHost, TransferStream);
            ...
            // DELOADING: phase 1
                // D2H: A[depth]        //S1
                    A_list[depth] = new T[n*layers[depth]+1];
                    cudaMemcpyAsync(A_list[depth], d_b2_A, n*A_bytesizes[depth] + T_size, cudaMemcpyDeviceToHost, TransferStream);

                // <<<prop>>>(depth)    //S2
                    if (cudaSuccess!= cudaMemset(d_b1_A, 0, n*A_bytesizes[depth+1] + T_size) ) {throw std::runtime_error("CUDA memset failed");};
                    gridDim.x = layers[depth+1]+1;       //i.e. blockIdx.y goes from 0 -> n-1. blockIdx.x goes from 0 -> p/word_size -1    where p is the #cols of the next layer 
                    propagate<T><<<gridDim,blockDim,0,ProcessStream>>>(d_b2_A, d_b1_W, d_b1_A);
            ...
            // DELOADING: phase 2
                // D2H: A[depth+1]      //S1
                    A_list[depth+1] = new T[n*layers[depth+1]+1];
                    cudaMemcpyAsync(A_list[depth+1], d_b1_A, n*A_bytesizes[depth+1] + T_size, cudaMemcpyDeviceToHost, TransferStream);
        ...
        if (cudaSuccess!= cudaMalloc(&d_A, n*A_max + T_size) ) {throw std::runtime_error("CUDA memory allocation failed");};     //this is where malloc error coming from on second run 
        if (cudaSuccess!= cudaMalloc(&d_W, W_max) ) {throw std::runtime_error("CUDA memory allocation failed");};
        if (cudaSuccess!= cudaMalloc(&d_O, n*A_max + T_size) ) {throw std::runtime_error("CUDA memory allocation failed");};
