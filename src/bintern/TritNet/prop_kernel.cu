...
__global__ void propagate(T* d_A, T* d_W, T* d_c){
    ...
    //new indexing:
    int A_idx = 1 + blockIdx.y*d_m;                                 //assert(A_idx < 4);
    ...
    //activation
    sigma(thread_result) ? thread_result = (1<<(d_WORD_SIZE-1-threadIdx.x)) : thread_result = 0;     //left-most bit as MSB: 2^(word_size-threadIdx.x). left-most bit as LSB: (2^threadIdx.x).
    ...
    atomicOr(&(d_c[1 + blockIdx.x + (d_p/d_WORD_SIZE)*blockIdx.y]),  thread_result);
   
    d_c[0] = d_p/d_WORD_SIZE; //maybe make this only occur for one thread, to prevent delays from everything trying to access same memory
